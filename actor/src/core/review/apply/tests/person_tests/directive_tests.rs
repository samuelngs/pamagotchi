...
#[tokio::test]
async fn apply_review_can_create_current_channel_directive() {
    ...
    let channel = ctx.messages[0].channel_id();
    ...
    let directives = store
        .get_directives_for_context(&person, &RelationshipStanding::Default, Some(&channel))
        .await
        .unwrap();
    assert_eq!(directives.len(), 1);
    assert_eq!(
        directives[0].scope.scope_value().as_deref(),
        Some(channel.0.as_str())
    );
    ...
    let result = apply(
        &json!({
            "directives": [{
                ...
                "directive": "Use another channel's norm."
            }, {
                "scope": "person",
